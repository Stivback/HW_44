...
public class Linear {
	...
	// validate (41) elements presence - when element has size more then 0 px
	public static boolean isPresent(By by) {
		if (driver.findElements(by).size() > 0) return true; 
		else return false;
		}

	// validate (10) elements visibility - when element has size more then 0 px
	public static boolean isVisible(By by) {
		if ((driver.findElements(by).size() > 0) && driver.findElement(by).isDisplayed()) return true; 
		else return false;
		}
	
	// Elements for validation for pages: signup & confirmation
	...
	public static void main(String[] args) {
		...
		// Performance test (start) in Milli Seconds
